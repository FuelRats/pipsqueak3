...
class TestGracefulErrors(object):
    ...
    def test_make_graceful(self, monkeypatch):
        ...
        expected = 'Oh noes! Stinky special_cheese encountered! please contact a tech! ' \
                   'Reference code faffaec4'

        # hack CHEESES to only have one item to make test deterministic
        monkeypatch.setattr("src.packages.graceful_errors.graceful_errors.CHEESES",
                            ["special_cheese"])
