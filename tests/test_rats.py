...
class TestRat(TestCase):
    ...
    def setUp(self):
        ...
        # make a rat
        self.my_rat = Rat(uuid=self.some_id, name="UNIT_TEST", platform=Platforms.PC)

    def test_new_instance(self):
        """
        Verifies creating a new instance of `Rat` functions as expected
    ...
    def test_update_cache_on_new_instance(self):
        ...
        self.assertEqual(Rat.cache_by_id[self.some_id], self.my_rat)
        self.assertEqual(Rat.cache_by_name["UNIT_TEST"], self.my_rat)
    ...
    def test_name_good_type(self):
        """
        Verifies `Rat.name` can be set when given good data
        """
    ...
    def test_name_bad_type(self):
        """
        Verifies `Rat.name` raises a type error when someone throws garbage
         at it.
        """
    ...
    def test_uuid_good_type(self):
        """
        Verifies `Rat.uuid` can me set when given good data.
        """
    ...
    def test_uuid_bad_types(self):
        """
        Verifies `Rat.uuid` raises a TypeError when someone throws garbage
        at it.
        """
...
class TestRatsPyTest(object):
    ...
    async def test_find_rat_by_name_bad_type(self, garbage):
        ...
        """
        with pytest.raises(TypeError):
            await Rat.get_rat_by_name(name=garbage)

        with pytest.raises(TypeError):
            await Rat.get_rat_by_name(name="foo", platform=garbage)
    ...
    async def test_find_rat_by_name_not_in_cache_and_no_API(self):
        ...
        """
        result = await Rat.get_rat_by_name(name="somenamethatdoesnotexist")
    ...
    async def test_get_rat_by_uuid_garbage(self, garbage):
        with pytest.raises(TypeError):
            await Rat.get_rat_by_uuid(uuid=garbage)
    ...
    async def test_get_rat_by_uuid_cache_miss(self):
        ...
        uuid = uuid4()
        assert await Rat.get_rat_by_uuid(uuid) is None

    @pytest.mark.asyncio
    async def test_get_rat_by_uuid_cache_hit(self, rat_good_fx: Rat):
        """Verifies Rat.get_rat_by_uuid returns the correct rat on cache hit"""
        mine = rat_good_fx
        found = await Rat.get_rat_by_uuid(mine.uuid)
    ...
    @pytest.mark.asyncio
    async def test_find_rat_by_name_existing(self, rat_good_fx: Rat):
        """
        Verifies that cached rats can be found by name
        """
        found_rat = await Rat.get_rat_by_name(name=rat_good_fx.name)
    ...
    async def test_find_rat_incorrect_platform(self):
        ...
        found_rat = await Rat.get_rat_by_name(name="UNIT_TEST", platform=Platforms.XB)
