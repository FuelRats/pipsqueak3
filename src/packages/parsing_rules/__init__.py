...
api_id = pyparsing.Word(initChars=pyparsing.hexnums+"@", bodyChars=pyparsing.hexnums + "-", min=36, max=37)
""" matches a well formed UUID4"""

case_number = (
    # may lead with 'case'
    api_id.setParseAction(
        lambda token: UUID(token[0][1:]) if token[0].startswith("@") else UUID(token[0])
    )
