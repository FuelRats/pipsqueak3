...
async def cmd_ratid(context: Context):
    ...
    results = await context.bot.api_handler.get_rat(key=tokens.subject[0],
                                                    impersonation=context.user.account)

    if not results:
        return await context.reply(f"no rats found for {tokens.subject[0]!r}.")

    return await context.reply(', '.join(f"{rat.name} on {rat.platform.colorize()}" for rat in results))
